...
consumeAndProcess msg handler = do
  case eitherDecode' (msgBody msg) of
    Left err -> withMsgAndErr msg err $ do
      ...
      return False
    Right payload -> do
      result <- try (handler payload)
      case result of
        Left err -> withMsgAndErr msg (displayException (err :: SomeException)) $ do
...
withMsgAndErr msg err action =
  katipAddContext (sl "mqMsg" (show msg) <> sl "error" err) action