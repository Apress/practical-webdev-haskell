...
initQueue state@(State pubChan _) qName excName routingKey = do
  initExchange state excName
  void $ declareQueue pubChan (newQueue { queueName = qName })
  bindQueue pubChan qName excName routingKey

initExchange :: State -> Text -> IO ()
initExchange (State pubChan _) excName = do
  let exchange = newExchange  { exchangeName = excName
                              , exchangeType = "topic" }
  declareExchange pubChan exchange

initConsumer :: State -> Text -> (Message -> IO Bool) -> IO ()
initConsumer (State _ conChan) qName handler =
  void . consumeMsgs conChan qName Ack $ \(msg, env) -> void . fork $ do
    result <- handler msg
